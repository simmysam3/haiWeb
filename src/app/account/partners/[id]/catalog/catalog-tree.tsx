      ...
      {classes.length === 0 ? (
        ...
        <div className="rounded border border-slate/20 bg-white divide-y divide-slate/10">
          {classes.map((klass) => {
            ...
            const classDeepLink = `/account/sonar/compliance/requests/new-nomination?vendor=${encodeURIComponent(vendorId)}&class_id=${encodeURIComponent(klass.class_id)}`;
            return (
                    ...
                    {productsState?.loaded && productsState.products.length > 0 && (
                      <ul className="space-y-1">
                        {productsState.products.map((product) => {
                          ...
                          const productDeepLink = `/account/sonar/compliance/requests/new-nomination?vendor=${encodeURIComponent(vendorId)}&product=${encodeURIComponent(product.external_product_id)}`;
                          return (
                            <li
