...
describe('DirectionTabs — v.1.37 IA (v.1.41: + Declined tab)', () => {
  it('renders four tabs with counts on me/them/all (no count chip on declined)', () => {
    ...
    expect(screen.getByRole('tab', { name: /Awaiting me/ })).toBeInTheDocument();
    expect(screen.getByRole('tab', { name: /Awaiting them/ })).toBeInTheDocument();
  ...
  it('clicking Them pushes ?direction=them', () => {
    ...
    fireEvent.click(screen.getByRole('tab', { name: /Awaiting them/ }));
  ...
  it('clicking Me clears the direction param (me is implicit default)', () => {
    ...
    render(<DirectionTabs awaitingMeCount={0} awaitingThemCount={0} totalCount={0} />);
    fireEvent.click(screen.getByRole('tab', { name: /Awaiting me/ }));
