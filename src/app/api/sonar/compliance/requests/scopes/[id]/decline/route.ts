...
/**
 * POST /api/sonar/compliance/requests/scopes/[id]/decline
 *
 * BFF passthrough to haiCore POST /sonar/compliance/requests/scopes/:id/decline.
 * Body: { reason?: string } (≤ 2000 chars; surfaced to the counterparty per Q7 R-4).
...
export const POST = withHaiCore<{ id: string }>(async ({ client, request, params }) => {
  ...
  try {
    ...
    console.warn('[BFF requests/scopes/decline] body-read failed', e);
  ...
  try {
    haiCoreRes = await client.fetchRaw(
      `/sonar/compliance/requests/scopes/${encodeURIComponent(params.id)}/decline`,
      {
        method: 'POST',
    ...
    console.warn('[BFF requests/scopes/decline] network error', e);
    return NextResponse.json(
