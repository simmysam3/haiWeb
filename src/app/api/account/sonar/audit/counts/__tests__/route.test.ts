...
vi.mock('@/lib/with-hai-core', () => ({
  withHaiCore: (handler: any) => async (req: NextRequest, _ctx?: any) => {
    const client = (globalThis as any).__mockCountsClient;
...
describe('GET /api/account/sonar/audit/counts', () => {
  beforeEach(() => {
    vi.resetModules();
    (globalThis as any).__mockCountsClient = {
  ...
  it('scheduled_count counts only audit+enabled templates (ignores disabled audit and non-audit templates)', async () => {
    (globalThis as any).__mockCountsClient.listRunTemplates.mockResolvedValue({
      templates: [
    ...
    (globalThis as any).__mockCountsClient.listAuditRuns.mockResolvedValue({ runs: [] });
  ...
  it('in_flight_count equals the number of running runs returned', async () => {
    (globalThis as any).__mockCountsClient.listRunTemplates.mockResolvedValue({ templates: [] });
    (globalThis as any).__mockCountsClient.listAuditRuns.mockResolvedValue({
      runs: [
  ...
  it('if listAuditRuns rejects, in_flight_count is 0 and scheduled_count still computes (no 500)', async () => {
    ...
    (globalThis as any).__mockCountsClient.listAuditRuns.mockRejectedValue(new Error('haiCore timeout'));
