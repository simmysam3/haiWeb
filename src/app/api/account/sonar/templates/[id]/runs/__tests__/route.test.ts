...
type MockRouteCtx = { params: Promise<Record<string, string>> };
...
describe('GET /api/account/sonar/templates/[id]/runs', () => {
  ...
  it('watcher template: dispatches to listWatcherRuns', async () => {
    ...
    globalThis.__mockClient.listWatcherRuns.mockResolvedValue({
    ...
    const body = await res.json();
    expect(body.runs).toHaveLength(1);
    expect(globalThis.__mockClient.listWatcherRuns).toHaveBeenCalledWith({
      template_id: 'tB',
      limit: 200,
    });
  ...
  it('phantom_demand template: dispatches to listPhantomDemandRuns (bare array; created_at → triggered_at)', async () => {
    ...
    globalThis.__mockClient.listPhantomDemandRuns.mockResolvedValue([
    ...
    expect(globalThis.__mockClient.listPhantomDemandRuns).toHaveBeenCalledWith({
      template_id: 'tP',
      limit: 200,
    });
    expect(globalThis.__mockClient.listAuditRuns).not.toHaveBeenCalled();
  ...
  it('returns slice limited to 25 most-recent runs', async () => {
    ...
    globalThis.__mockClient.listAuditRuns.mockResolvedValue({ runs: manyRuns });
    const res = await GET(makeReq('tA'), { params: Promise.resolve({ id: 'tA' }) });
    const body = await res.json();
