...
describe('DELETE /api/account/sonar/templates/[id] — haiCore error pass-through', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("passes a 409 RUNS_IN_FLIGHT error through with haiCore's status and body", async () => {
    deleteRunTemplate.mockRejectedValueOnce(
      Object.assign(new Error('haiCore DELETE /sonar/templates/abc: 409 {...}'), {
        status: 409,
    ...
    const res = await DELETE(
      new NextRequest('http://localhost/api/account/sonar/templates/abc?runs=delete', { method: 'DELETE' }),
