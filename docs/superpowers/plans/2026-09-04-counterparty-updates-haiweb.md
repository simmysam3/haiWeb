# Counterparty Updates — HaiWeb lane (v1.89) Implementation Plan
...
**Spec:** `docs/superpowers/specs/2026-09-04-counterparty-attribute-ownership-and-updates-design.md` (byte-identical copy of haiCore's canonical spec). Wire shapes: haiCore's plan `~/dev/hw/haiCore/docs/superpowers/plans/2026-09-04-counterparty-updates-haicore.md` (on haiCore main; the haiCore-cpsync worktree is gone — read the primary, never edit it) Task 1 and Task 7's route table; the shipped schemas are `~/dev/hw/haiCore/packages/protocol/src/counterparty-updates.ts` (protocol 3.82.0). Plan A's header (review round 1) records eight stated deviations from the spec; this plan follows them — in particular the decision body is `{ keep: 'mine' | 'theirs' } | { link: number }` (Plan A's `CounterpartyUpdateDecisionSchema` is the source of truth; the spec's `'<erp_ref>'` was illustrative), and no location-UPDATE verb ships this cycle (a matched location on take-theirs answers `not_permitted` and shows as dirty until the manual edit converges — the same alert path).

## Global Constraints

- Worktree `~/dev/hw/haiWeb-cpsync`, branch `v1.88-counterparty-updates` (a label only — the lane is in the v1.89 cycle), base `de85db2` with origin/master `0ff560f` (#187) merged in at `8e2f5c4` on 2026-09-07. Never touch `~/dev/hw/haiWeb` (the :3001 console's tree) or `~/dev/hw/haiCore` (the live Central's tree).
- Release: HaiWeb **v1.89** cycle, slot **v1.89 PR 3** (allocated by agent1 2026-09-07, `~/dev/hw/ALLOCATION-2026-09-04-counterparty-updates-agent1.md`; PR title shape "v1.89 — counterparties: …"); no package.json bump (HaiWeb releases follow the cycle label); no new D / §L / PG / protocol numbers (consumes D-208/D-209; route gating cites D-211). Merge order: after haiCore v1.88.0 and haiClient 1.89.0 — both tagged 2026-09-05/06, satisfied.
...
- Tests: `~/dev/hw/vitest-lock.sh npx vitest run <file> --maxWorkers=3` (one vitest on the machine at a time; this tree's vitest rejects `--minWorkers` — measured 2026-09-07). Gate at HOLD: `npm run build` (0) · `vitest run` (0 failed) · `npx playwright test --list` (0 type errors).
- Commits end with `Co-Authored-By: Claude Fable 5.1 <noreply@anthropic.com>`; never push (owner's word in session hw-2e's window).
...
// decision body mirrors Plan A's CounterpartyUpdateDecisionSchema (`~/dev/hw/haiCore/packages/protocol/src/counterparty-updates.ts` `CounterpartyUpdateDecisionSchema`, :147-151 on haiCore main) — that schema is the source of truth, not the spec's illustrative '<erp_ref>' string
...
- [ ] **Step 2: Gate** (record counts): `npm run build` (0) · `~/dev/hw/vitest-lock.sh npx vitest run --maxWorkers=3` (0 failed) · `npx playwright test --list` (0 type errors). The protocol the primary haiCore carries at that moment is what the build sees; this feature imports nothing new from it. Census: `git -C /Users/samfleming/dev/hw/haiWeb-cpsync diff origin/master...HEAD -- src | grep -c '^+.*@haiwave/protocol'` must be 0 added import lines; present control: `grep -rn "@haiwave/protocol" src | wc -l` returns the existing imports (measured 293 today). This census is the reason the 3.82.0-resolved gate (protocol via `file:../haiCore/packages/protocol`, the PRIMARY tree) is binding.
- [ ] **Step 3: Visual walk** — NOT from this worktree and NOT on :3002 (:3002 is agent1's second console off the primary build since 2026-09-07; message hw-48 before any build in the primary). Per the owner's word of 2026-09-07 the walk runs in the owner's Chrome on :3001 AFTER merge and agent1's refresh (§7 items 2–7 of the archived haiClient PR body): the seven tabs; the Counterparty updates tab against the merged haiCore (rows seeded by the haiClient live proof); the alert text verbatim; the profile locations round-trip (save, reload, plants persist). Screenshots into `.superpowers/walk-v189/` (untracked; preserve). **This walk is blocked until haiCore v1.88.0 AND haiClient 1.89.0 are merged and the haiClient live proof has seeded rows on central** — the Counterparty updates tab and the alert have nothing to render before then. Do not treat this as a gate condition for this lane's own HOLD; record it as an owed check.
- [ ] **Step 4: HOLD record** `.superpowers/PR-BODY-haiweb-v1.89.md` (the haiClient PR body's shape): tab, BFF, profile locations, tab registration (no rename), copy census; gate counts; walk screenshots; merge order last; release PR slot `v1.89 PR 3`. Record the Step 3 visual walk as an OWED CHECK (blocked on haiCore + haiClient merging first) rather than a gate condition for this HOLD. Then STOP and report to the owner. **Push and the release PR only on the owner's word in session hw-2e's window; agent1 (hw-48) merges and refreshes :3001/:3002 on its own word.**
